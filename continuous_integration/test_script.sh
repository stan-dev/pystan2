...
if [[ $TRAVIS_OS_NAME == 'linux' ]]; then
    DISPLAY=:99.0 JOBLIB_MULTIPROCESSING=0 nosetests -w /tmp --exclude=$TEST_SUITE_REGEX pystan
fi

if [[ $TRAVIS_OS_NAME == 'osx' ]]; then
    # skip DISPLAY setting and hope for the best
    JOBLIB_MULTIPROCESSING=0 nosetests -w /tmp --exclude=$TEST_SUITE_REGEX pystan
fi
